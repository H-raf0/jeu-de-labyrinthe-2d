
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <math.h>
#include <FL/math.h>
typedef struct cmpl {
    float r;
    float i;
} Complex;

Complex rotation_img(Complex z, Complex z0 , float angle){
    Complex resultat;
    float x = z.r;
    float y = z.i;
    float x0 = z0.r;
    float y0 = z0.i;
    float theta = M_PI * angle / 180;

    resultat.r = cos(theta)*(x - x0) - sin(theta)*(y - y0) + x0;
    resultat.i = sin(theta)*(x - x0) + cos(theta)*(y - y0) + y0;
    return resultat;
}

Complex transformation_inverse(Complex z, Complex z0, float Angle_max, int d0, int d1, int d_max){
    float x = z.r;
    float y = z.i;
    float x0 = z0.r;
    float y0 = z0.i;

    float mod_z = sqrt((x - x0)*(x - x0) + (y - y0)*(y - y0));
    float theta_max = M_PI * Angle_max / 180;

    if (mod_z <= d0){
        return z;
    } else if (mod_z <= d_max){
        float angle = - theta_max / (d_max - d0) * (mod_z - d0);
        return rotation_img(z, z0, angle);
    } else if (mod_z <= d1){
        float angle = - (theta_max / (d_max - d1) * (mod_z - d_max) + theta_max);
        return rotation_img(z, z0, angle);
    } else {
        return z;
    }
}

#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <FL/math.h>


void generate_image(const char* filename,int width,int height, int apply_transform) {
 FILE* f = fopen(filename, "wb");
 if (!f) {
 perror("fopen");
 exit(1);
 }
 fprintf(f, "P6\n%d %d\n255\n", WIDTH, HEIGHT);

 Complex z0 = {WIDTH / 2.0, HEIGHT / 2.0};
 float Angle_max = 45;
 float d0 = 20, dmax = 100, d1 = 200;

 for (int y = 0; y < height; y++) {
 for (int x = 0; x < width; x++) {
 Complex z_prime = {x, y};
 Complex z_result = z_prime;
 if (apply_transform) {
 z_result = transformation_inverse(z_prime, z0, Angle_max, d0, d1, dmax);
 }

 int xi = (int)z_result.r;
 int yi = (int)z_result.i;

 // test de bord
 unsigned char r=0, g=0, b=0;
 if (xi >= 0 && xi < width && yi >= 0 && yi < height) {
 if ((xi/50 + yi/50) % 2 == 0) {
 r = 255;
 } else {
 g = 255;
 }
 }
 fputc(r, f);
 fputc(g, f);
 fputc(b, f);
 }
 }

 fclose(f);
}

int main() {
 int width = 512, height = 512;

 generate_image("original.ppm", width, height, 0);
 generate_image("transformee.ppm", width, height, 1);

 printf("Images générées.\n");
 return 0;
}