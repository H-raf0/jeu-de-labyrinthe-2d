...
void load_sounds(AudioData* audio) {
    audio->menuMusic = Mix_LoadMUS("assets/music/menu_music.mp3");
    if (!audio->menuMusic) { printf("Erreur dans load_sounds"); return; }

    audio->gameMusic = Mix_LoadMUS("assets/music/game_music.mp3");
    if (!audio->gameMusic) { printf("Erreur dans load_sounds"); return; }

    audio->o2Sound = Mix_LoadWAV("assets/audio/o2_collect.wav");
    if (!audio->o2Sound) { printf("Erreur dans load_sounds"); return; }

    audio->victorySound = Mix_LoadWAV("assets/audio/victory.wav");
    if (!audio->victorySound) {
        printf("Failed to load victory sound: %s\n", Mix_GetError());
    }

    audio->failureSound = Mix_LoadWAV("assets/audio/failure.wav");
