...
int main(int argc, char* argv[]) {
    ...
    // Les positions sont des 'float' pour conserver la précision et éviter les saccades/découpages
    float x1[NUM_LAYERS], x2[NUM_LAYERS];

    // Chargement des textures et initialisation des positions/vitesses
    char filename[16];
    for (int i = 0; i < NUM_LAYERS; i++) {
        ...
        // Vitesse plus lente pour l'arrière-plan (i=0), plus rapide pour le premier plan (i=5)
        speeds[i] = 0.5f + (float)i * 0.75f;
        
        // Initialisation des positions des deux copies pour chaque couche
        x1[i] = 0.0f;
    ...
    // Zones de clic pour les flèches de difficulté
    ...
    while (running) {
        ...
        for (int i = 0; i < NUM_LAYERS; i++) {
            if (layers[i]) {
                // Le calcul de position se fait en float pour une précision maximale
                x1[i] -= speeds[i];
                x2[i] -= speeds[i];

                // Si une copie sort de l'écran, on la "téléporte" à la suite de l'autre
                if (x1[i] <= -SCREEN_WIDTH) { x1[i] = x2[i] + SCREEN_WIDTH; }
                if (x2[i] <= -SCREEN_WIDTH) { x2[i] = x1[i] + SCREEN_WIDTH; }

                // On convertit la position en (int) seulement au moment du dessin
        ...
        SDL_Delay(16); // Vise environ 60 images/seconde pour un mouvement fluide
    }
